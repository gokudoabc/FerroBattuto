<div>
	...
	$sqlContando = $mySQL->runQuery("SELECT * FROM fer_galeria WHERE idcategoria='$id' AND status='publicado'");
	$rsConta = mysql_fetch_array($sqlContando);
		if($rsConta){
			$sqlprodutos = $mySQL->runQuery("SELECT * FROM fer_galeria WHERE idcategoria='$id' AND status='publicado'");
	...
	<div>  
		...
		echo "<span style='color:#fff;'>Nenhuma imagem cadastrada.</span>";
	}
