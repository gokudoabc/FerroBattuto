<br/><br/><br/>
<form id="form1" name="form1" method="post" action="servicos/action.php">      
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td><h1><em>Servi&ccedil;os</em>
        <a href="principal.php?type=sinserir" class="bnt_inserir"><img src="_img/bnt_inserir.gif" width="73" height="22" border="0" /></a> </h1></td><?php include("_includes/exportar.php"); ?>
          </tr>
        <tr>
          <td height="30"><?php $Funcoes->contagem_registro($_GET['type'],'forteap_servicos');  ?> <input type="hidden" name="type" id="type" value="<?php echo $_GET['type']; ?>" /></td></td>
          <td>&nbsp;</td>
          <td>&nbsp;</td>
        </tr>
        <tr>
      ...
      <table width="100%" cellspacing="0">
		...
		if (!$page) {
			...
			$sqlnoticia = $mySQL->runQuery("SELECT * FROM forteap_servicos ".$where." ");
				$b=mysql_num_rows($sqlnoticia);
					$sqlnoticia = $mySQL->runQuery("SELECT * FROM forteap_servicos ".$where." LIMIT ". $inicio . ",".TOTAL_PAGINA."");
						//calculo das páginas													
		...
		?>
          <tr valign="top" class="post<?php echo $rsnoticia['idservicos']; ?> fundo<?php echo $i%2 ?>" onmouseover="posts('<?php echo $rsnoticia['idservicos']; ?>')" onmouseout="posts_out('<?php echo $rsnoticia['idservicos']; ?>')">
          
            <th valign="middle" scope="row"><input type="checkbox" name="id[]" value="<?php echo $rsnoticia['idservicos']; ?>" /></th>
            <td height="40" valign="middle">
            	<strong><a class="row-title" href="principal.php?type=edservico&id=<?php echo $rsnoticia['idservicos']; ?>" title="Edit &ldquo;TEste&rdquo;"><?php echo $rsnoticia['titulo'];?></a></strong>
                <?php $Funcoes->opcoes_post($rsnoticia['idservicos'],$rsnoticia['status'],'servicos/action.php','edservico'); ?>
			</td>
