...
<form id="form1" name="form1" method="post" action="noticia/action_noticia.php">      
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td><h1><em>Lista de clientes</em>
        <a href="principal.php?type=llista" class="bnt_inserir"><img src="_img/bnt_inserir.gif" width="73" height="22" border="0" /></a> </h1></td><?php include("_includes/exportar.php"); ?>
          </tr>
        <tr>
          <td height="30"><?php $Funcoes->contagem_registro($_GET['type'],'forteap_listaclientes');  ?> <input type="hidden" name="type" id="type" value="<?php echo $_GET['type']; ?>" /></td>
          <td>&nbsp;</td>
          <td>&nbsp;</td>
        </tr>
        <tr>
      ...
      <table width="100%" cellspacing="0">
        <thead>
          <tr style="background:url(_img/bg_area.gif) repeat-x; color:#000;">
            ...
            <th width="36%" height="30" align="left">Nome / Cliente</th>
		...
		if (!$page) {
			...
			$sqlnoticia = $mySQL->runQuery("SELECT * FROM forteap_listaclientes ".$where." ");
				$b=mysql_num_rows($sqlnoticia);
					$sqlnoticia = $mySQL->runQuery("SELECT * FROM forteap_listaclientes ".$where." LIMIT ". $inicio . ",".TOTAL_PAGINA."");
						//calculo das páginas													
		...
		?>
          <tr valign="top" class="post<?php echo $rsnoticia['idlista']; ?> fundo<?php echo $i%2 ?>" onmouseover="posts('<?php echo $rsnoticia['idlista']; ?>')" onmouseout="posts_out('<?php echo $rsnoticia['idlista']; ?>')">
          
            <th valign="middle" scope="row"><input type="checkbox" name="id[]" value="<?php echo $rsnoticia['idlista']; ?>" /></th>
            <td height="40" valign="middle">
            	<strong><a class="row-title" href="principal.php?type=edlista&id=<?php echo $rsnoticia['idlista']; ?>" title="Edit &ldquo;TEste&rdquo;"><?php echo $rsnoticia['nome'];?></a></strong>
                <?php $Funcoes->opcoes_post($rsnoticia['idlista'],$rsnoticia['status'],'lista/action.php','edlista'); ?>
			</td>
