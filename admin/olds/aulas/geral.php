...
<form id="form1" name="form1" method="post" action="aulas/action.php">      
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td><h1><em>Aulas médicas - Profissionais de sa&uacute;de</em>
        <a href="principal.php?type=auinserir" class="bnt_inserir"><img src="_img/bnt_inserir.gif" width="73" height="22" border="0" /></a> </h1></td><?php include("_includes/exportar.php"); ?>
          </tr>
        <tr>
      ...
      <table width="100%" cellspacing="0">
		...
		if (!$page) {
			...
			$sqlnoticia = $mySQL->runQuery("SELECT * FROM  neo_aulas ".$where." ");
				$b=mysql_num_rows($sqlnoticia);
					$sqlnoticia = $mySQL->runQuery("SELECT * FROM  neo_aulas ".$where." LIMIT ". $inicio . ",".TOTAL_PAGINA."");
						//calculo das páginas													
		...
		?>
          <tr valign="top" class="post<?php echo $rsnoticia['idaulas']; ?> fundo<?php echo $i%2 ?>" onmouseover="posts('<?php echo $rsnoticia['idaulas']; ?>')" onmouseout="posts_out('<?php echo $rsnoticia['idaulas']; ?>')">
          
            <th valign="middle" scope="row"><input type="checkbox" name="id[]" value="<?php echo $rsnoticia['idaulas']; ?>" /></th>
            <td height="40" valign="middle">
            	<strong><a class="row-title" href="principal.php?type=edaulas&id=<?php echo $rsnoticia['idaulas']; ?>" title="Editar"><?php echo $rsnoticia['titulo'];?></a></strong>
                <?php $Funcoes->opcoes_post($rsnoticia['idaulas'],$rsnoticia['status'],'aulas/action.php','edaulas'); ?>
			</td>
