...
<form id="form1" name="form1" method="post" action="noticia/action_noticia.php">      
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td><h1><em>Receitas</em>
        <a href="principal.php?type=rinserir" class="bnt_inserir"><img src="_img/bnt_inserir.gif" width="73" height="22" border="0" /></a> </h1></td><?php include("_includes/exportar.php"); ?>
          </tr>
        <tr>
          <td height="30"><?php //$Funcoes->contagem_registro($_GET['type'],'for_receitas');  ?>
      ...
      <table width="100%" cellspacing="0">
		...
		if (!$page) {
			...
			$sqlnoticia = $mySQL->runQuery("SELECT * FROM for_receitas ".$where." ");
				$b=mysql_num_rows($sqlnoticia);
					$sqlnoticia = $mySQL->runQuery("SELECT * FROM for_receitas ".$where." LIMIT ". $inicio . ",".TOTAL_PAGINA."");
						//calculo das páginas													
		...
		?>
          <tr valign="top" class="post<?php echo $rsnoticia['idreceita']; ?> fundo<?php echo $i%2 ?>" onmouseover="posts('<?php echo $rsnoticia['idreceita']; ?>')" onmouseout="posts_out('<?php echo $rsnoticia['idreceita']; ?>')">
          
            <th valign="middle" scope="row"><input type="checkbox" id="id_campos" name="id[]" value="<?php echo $rsnoticia['idreceita']; ?>" /></th>
            <td height="40" valign="middle">
            	<strong><a class="row-title" href="principal.php?type=edreceitas&id=<?php echo $rsnoticia['idreceita']; ?>" title="Edit &ldquo;TEste&rdquo;"><?php echo $rsnoticia['titulo'];?></a></strong>
                <?php $Funcoes->opcoes_post($rsnoticia['idreceita'],$rsnoticia['status'],'receitas/action.php','edreceitas'); ?>
			</td>
