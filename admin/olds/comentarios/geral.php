<br/><br/><br/>
<form id="form1" name="form1" method="post" action="comentarios/action.php">      
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td><h1><em>Comentarios - Profissionais de sa&uacute;de</em>
      ...
      <table width="100%" cellspacing="0">
        <thead>
          <tr style="background:url(_img/bg_area.gif) repeat-x; color:#000;">
            ...
            <th width="15%" height="30" align="left">Titulo</th>
		...
		if (!$page) {
			...
			$sqlnoticia = $mySQL->runQuery("SELECT * FROM  neo_comentarios ".$where." ");
				$b=mysql_num_rows($sqlnoticia);
					$sqlnoticia = $mySQL->runQuery("SELECT * FROM  neo_comentarios ".$where." LIMIT ". $inicio . ",".TOTAL_PAGINA."");
						//calculo das páginas													
		...
		?>
          <tr valign="top" class="post<?php echo $rsnoticia['idcoment']; ?> fundo<?php echo $i%2 ?>" onmouseover="posts('<?php echo $rsnoticia['idcoment']; ?>')" onmouseout="posts_out('<?php echo $rsnoticia['idcoment']; ?>')">
          
            <th valign="middle" scope="row"><input type="checkbox" name="id[]" value="<?php echo $rsnoticia['idcoment']; ?>" /></th>
            <td height="40" valign="middle">
            	<strong><a class="row-title" href="principal.php?type=edcomentarios&id=<?php echo $rsnoticia['idcoment']; ?>" title="Editar"><?php echo $rsnoticia['titulo'];?></a></strong>
                <?php $Funcoes->opcoes_post($rsnoticia['idcoment'],$rsnoticia['status'],'comentarios/action.php','edcomentarios'); ?>
			</td>
