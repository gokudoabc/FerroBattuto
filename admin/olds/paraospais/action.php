...
switch ($_GET['status']) {		
	...
	if($_GET['opcao']=='1') {
			foreach($ids_explode as $values) {
				if($values !='') {
				$sql = $mySQL->runQuery("UPDATE for_pais SET  status='lixo' WHERE idartigo='".$values."'");
				}
			}			
		} else {	
		$sql = $mySQL->runQuery("UPDATE for_pais SET  status='lixo' WHERE idartigo='".$_GET['id']."'");
		}
			header("Location:../principal.php?type=para_os_pais");					
	break;
	...
	if($_GET['opcao']=='1') {
			foreach($ids_explode as $values) {
				if($values !='') {
				$sql = $mySQL->runQuery("UPDATE for_pais SET  status='inativo' WHERE idartigo='".$values."'");	
				}
			}			
		} else {	
		$sql = $mySQL->runQuery("UPDATE for_pais SET  status='inativo' WHERE idartigo='".$_GET['id']."'");	
		}
			header("Location:../principal.php?type=para_os_pais");		
	break;
	...
	if($_GET['opcao']=='1') {
			foreach($ids_explode as $values) {
				if($values !='') {
				$sql = $mySQL->runQuery("UPDATE for_pais SET  status='publicado' WHERE idartigo='".$values."'");
				}
			}			
		} else {	
		$sql = $mySQL->runQuery("UPDATE for_pais SET  status='publicado' WHERE idartigo='".$_GET['id']."'");
		}
			header("Location:../principal.php?type=para_os_pais");		
	break;
	case 'restaurar':
	...
	break;
	case 'deletar':
		$sql = $mySQL->runQuery("DELETE FROM for_pais WHERE idartigo ='".$_GET['id']."'");
			header("Location:../principal.php?type=para_os_pais");		
	break;
...
if($_POST['opt']=='editar') {
	...
	$sql = $mySQL->runQuery("UPDATE for_pais SET  
			titulo='".$_POST['titulo']."',
			resumo='".$_POST['resumo']."',
			imagem='".$imagem."',
			datamodificacao=NOW(),
			...
			WHERE idartigo='".$_POST['idartigo']."'");
				...
				header("Location:../principal.php?type=edpais&id=".$_POST['idartigo']."");
				exit;	
