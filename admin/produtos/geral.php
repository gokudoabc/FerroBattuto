<br/><br/><br/>
<form id="form1" name="form1" method="post" action="produtos/action.php">      
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td><h1><em>Produtos</em>
        <a href="principal.php?type=iprodutos" class="bnt_inserir"><img src="_img/bnt_inserir.gif" width="73" height="22" border="0" /></a> </h1></td><?php include("_includes/exportar.php"); ?>
          </tr>
        <tr>
          <td height="30"><?php //$Funcoes->contagem_registro($_GET['type'],'clt_produtos');  ?><input type="hidden" name="type" id="type" value="<?php echo $_GET['type']; ?>" /></td>
          <td>&nbsp;</td>
          <td>&nbsp;</td>
        </tr>
        <tr>
      ...
      <table width="100%" cellspacing="0">
		...
		if (!$page) {
			...
			$sqlnoticia = $mySQL->runQuery("SELECT * FROM clt_produtos ".$where." ");
				$b=mysql_num_rows($sqlnoticia);
					$sqlnoticia = $mySQL->runQuery("SELECT * FROM clt_produtos ".$where." LIMIT ". $inicio . ",".TOTAL_PAGINA."");
						//calculo das páginas													
		...
		?>
          <tr valign="top" class="post<?php echo $rsnoticia['idproduto']; ?> fundo<?php echo $i%2 ?>" onmouseover="posts('<?php echo $rsnoticia['idproduto']; ?>')" onmouseout="posts_out('<?php echo $rsnoticia['idproduto']; ?>')">
          
            <th valign="middle" scope="row"><input type="checkbox" name="id[]" value="<?php echo $rsnoticia['idproduto']; ?>" /></th>
            <td height="40" valign="middle">
            	<strong><a class="row-title" href="principal.php?type=edprodutos&id=<?php echo $rsnoticia['idproduto']; ?>" title="Editar"><?php echo $rsnoticia['nome'];?></a></strong>
                <?php $Funcoes->opcoes_post($rsnoticia['idproduto'],$rsnoticia['status'],'noticias/action.php','edprodutos'); ?>
			</td>
