...
<form id="form1" name="form1" method="post" action="noticias/action.php">      
      <table width="100%" border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td><h1><em>Categorias</em>
        <a href="principal.php?type=icategorias" class="bnt_inserir"><img src="_img/bnt_inserir.gif" width="73" height="22" border="0" /></a> </h1></td><?php include("_includes/exportar.php"); ?>
          </tr>
        <tr>
          <td height="30"><?php //$Funcoes->contagem_registro($_GET['type'],'clt_acontece');  ?><input type="hidden" name="type" id="type" value="<?php echo $_GET['type']; ?>" /></td>
          <td>&nbsp;</td>
          <td>&nbsp;</td>
        </tr>
        <tr>
      ...
      <table width="100%" cellspacing="0">
		...
		if (!$page) {
			...
			$sqlnoticia = $mySQL->runQuery("SELECT * FROM clt_categorias ".$where." ");
				$b=mysql_num_rows($sqlnoticia);
					$sqlnoticia = $mySQL->runQuery("SELECT * FROM clt_categorias ".$where." LIMIT ". $inicio . ",".TOTAL_PAGINA."");
						//calculo das páginas													
		...
		?>
          <tr valign="top" class="post<?php echo $rsnoticia['idcategoria']; ?> fundo<?php echo $i%2 ?>" onmouseover="posts('<?php echo $rsnoticia['idcategoria']; ?>')" onmouseout="posts_out('<?php echo $rsnoticia['idcategoria']; ?>')">
            ...
            <td height="40" valign="middle">
            	<strong><a class="row-title" href="principal.php?type=edcategorias&id=<?php echo $rsnoticia['idcategoria']; ?>" title="Editar"><?php echo $rsnoticia['nome'];?></a></strong>
                <?php $Funcoes->opcoes_post($rsnoticia['idcategoria'],$rsnoticia['status'],'categorias/action.php','edcategorias'); ?>
			</td>
